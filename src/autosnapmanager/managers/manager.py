...
class Manager(ABC):
    ...
    def _init_method(self, system, param, method: Any, super_class) -> Union[ScreenCap, Match, Click]:
        """初始化方法类"""
        if method is None:
    ...
    @staticmethod
    def _set_default_method(system, param, super_class) -> Union[ScreenCap, Match, Click]:
        """设置方法默认值"""
        default_methods = DefaultMethods.get(super_class.__name__).get(system)
        map_table = CLASSMAP[super_class.__name__]

        return module(map_table, default_methods[0], system)(param) \
            if param else (
            module(map_table, default_methods[1], system)())
